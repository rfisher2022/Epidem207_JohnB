* Macro npar1way combines output from the univariate macro with p-values for Kruskal-Wallis or Mann-Whitney tests and organizes the
  ...
  Inputs: 
      ...
      Append = name of SAS output file containing estimates, confidence intervals, etc. to which the current results 
              should be appendend.  It is not necessary to supply out= in order to use append=.  The file referenced
              here may or may not exist.

	Last modified: July 16, 2010
   History		Oct 23, 2009		Fixed false error about Print=N and Out not specified
					Jul 16, 2010      Took out left over "POINT 4" message in log;
...
%include "&MacroDir\Univariate.sas";
%include "&MacroDir\Varlist.sas";
%include "&MacroDir\Words.sas";
%include "&MacroDir\Append.sas";
...
%macro NPar1way(dsname=,GroupVar=,Vars=,ByVars=,Print=Y,VarName=Y,Label=N,Dec=3,Out=,Append=);
	...
	* Check that either Print = Y or there is an output data set;
	%if &Print ne Y and &out= and &append= %then
		%do;
			%put;
   ...
   * Delete missing values of grouping variables.  Allow by groups based on missing values;
   ...
   run;
	...
	%if &out ne or &append ne %then
		%do;
			data _toappend (Type = 'NP1');
			   set _all;
				Length GroupVar $ 32 ByVars $ 100 Class $ 60;
				Class = putn(&GroupVar,vformat(&GroupVar));
				GroupVar = "&GroupVar";
				ByVars = "&ByVars";
			run;
		%end;
	...
	%if &append ne %then
		%do;
			%append(base=&append,dsname=_toappend)
		%end;
	data _all;
	...
	%if &Print = Y %then
		...
		%end;
	%ExitNPar1way:
	proc datasets nolist; delete _all _allnpar _kw _toappend _use _utemp _wt; quit; 
