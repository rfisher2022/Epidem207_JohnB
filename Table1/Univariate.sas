...
Last modified March 6, 2009	Variable Name of Type changed to _Type
              September 4, 2009 Error handling made consistent;
/* Macros used
%Let MacroDir = L:\BCU Macros\ForTesting;
	...
	* Check for alpha variables in the dependent list;
	...
	%do J = 1 %to &NVars;
		...
		data _null_;
		   set &dsname (obs = 1);
			_Type = VType(&Var);
			if _Type NE 'N' then call Symput('BadVar',1);
		run;
		%if &BadVar %then
			%do;
				%put;
