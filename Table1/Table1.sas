* Macro Table1 combines descriptive statistics and p-values in a typical Table 1 format where columns correspond
  ...
  Inputs:
      ...
      GroupVar = two or more level numeric or alphanumeric grouping variable.  Observations with missing values on
                 ...
                 This variable CAN NOT have the name cat. Program will abend.
      ...
      Median = MM|IQR|Y|N 
				   MM or Y = continuous variables are summarized with median (min-max).
				   IQR = continuous variables are summarized with median (q1-q3).
				 	N = median is not output
				  If both Mean and Median are requested, then there will be two lines of output per continuous variable.
		Total = R|C|N|RC
      ...
      KW = list of variables in FreqVars for which a Wilcoxon rank-sum (or Kruskal-Wallis) p-value should be used.
		FreqCell = N|RP|CP|N(RP)|N(CP)|RP(N)|CP(N)  RP = Row percent, CP = column percent. (default = N(RP))
	   Missing = Y|N. If Y, then frequencies for categorical variables will include missing values. Default is N.
		...
		Dec = A or integer. If an integer, then this is the number of decimal places used to display means and SDs.
				This value will apply to all continuous variables.  If Dec = A (the default), then the number of decimals
            is chosen automatically for each variable so that three significant figures are displayed. 
		Sig = number of significant digits to be displayed in means and SDs.  This parameter is applicable only if Dec = A. 
            Default is 3.
	 	Label = N|L|NL 
				 N = variable name, L = variable label. This parameter affects categorical variable labeling only. 
	...
	History:
	9-25-2009	Footnotes 1,2,3,4 changed to 3,4,5,5
	10-13-2009	Fixed error in check for alpha vars in KW list
	11-25-2009  Added options for number of decimals in output
   12-09-2009  Fixed problem with significant digits for negative means, medians, etc.
	12-21-2009  Fixed problem with significant digits for zero values.
	01-19-2010	Fixed "then then" error in code.
	01-29-2010  Fixed problem with p-values showing up for one level categorical variables
	03-11-2011  Fixed problem of message "Print=N and no output file"  when no grouping variable
	03-11-2011	Added ods listing close before proc freq than generates _getns data set
	05-06-2011	Added Sig parameter to macro call rather than the hardwired value of 3
	04-29-2015  Added ods graphics off statement
    04-07-2016  Corrected lengths in _freqs data set so that when appended variables have same length as in base file;



%macro Table1(DSName=,GroupVar=,NumVars=,FreqVars=,Mean=Y,Median=N,Total=C,P=W,Fisher=,KW=,FreqCell=N(RP),Missing=N,Print=Y,Dec=A,Sig=3,Label=L,Out=,Out1way=);
    
    ods graphics off;
    run;
     
	%Local BadVar Cat1 Cat2 Cat3 Cat4 Cat5 Cat6 Cat7 Cat8 Cat9 Cat10 CatFmt	Fish FisherKW GroupFmt HasChisq HasFisher HasMW
			 I K KWVar NBoth NCat NCells NFisher NFreqVars NGPs NGroups NKW NNote NNumVars NRows NVars ThisFreqVar ThisVar Val1 Val2 Val3 Val4
   ...
   * Check that there are no alpha variables in the numeric variable list;
	...
	%if &NNumVars > 0 %then
	   %do K = 1 %to &NNumVars;
	      ...
	      data _null_;
	         set &dsname (obs = 1);
	         Type = VType(&Var);
	         if Type NE 'N' then call symput('BadVar',1);
			run;
			%if &BadVar %then
				%do;
					%put;
	...
	* Check that Dec specification is legal;
	%if &Dec ne A and (&Dec <0 or &Dec > 7) %then
		%do;
			%put;
			%put ERROR: The decimal spcification must be 0 through 7 or A.;
			%put;
			%goto ExitTable1;
		%end;
   ...
   * Delete observations with missing values on the grouping variable;
   		...
   		%if &GroupVar ne _CONST %then
      		%do;
					* Get totals across rows;
					...
					data _Tabutemp2;
					   set _tabutemp2 (Keep = Variable Mean Median Min Max SD N Q1 Q3
						                rename = (N = TN Mean = TMean Median = TMedian Min = TMin Max = TMax SD = TSD Q1 = TQ1 Q3 = TQ3));
						Variable = upcase(Variable);
					run;
   		...
   		run; 
			...
			data _all2(drop = label rename = (Label2 = Label) );
      		set _all (rename = (Variable = Var)); by order;
				Length Label2 $ 80 PValue1 PValue2 $ 7 Variable $ 32 Total $ 60 Show1 $ 20 Show2 $ 20 Show3 $ 20;
      		...
      		array _a6 {&NGroups} median1-median&NGroups;
				...
				array _a8 {&NGroups} qone1-qone&NGroups;
				array _a9 {&NGroups} qthree1-qthree&NGroups;
      		if first.order then 
      		...
      		Retain Count n1-n&NGroups mean1-mean&NGroups sd1-sd&NGroups min1-min&NGroups max1-max&NGroups median1-median&NGroups
				       qone1-qone&NGroups qthree1-qthree&NGroups Variable PValue1 PValue2 Label2 Total TotN;
      		Count = Count+1;
				TotN = TotN + _a1{Count};
         		...
         		do;
						%if &Mean = Y %then
               			...
               			do i = 1 to &NGroups;		* Loop over the values of the grouping variable creating composite statistics for each value;
									%if &Dec = A %then
										%do;
											if _a2{i} = 0 then Show1 = '0.0';
											else
												do;
													nmean = floor(log10(abs(_a2{i}))) + 1 - &Sig;
													Show1 = round(10**(-nmean)*_a2{i})*10**nmean;
												end;
											if _a3{i} = 0 then Show2 = '0.0';
											else
												do;
													nsd = floor(log10(_a3{i})) + 1 - &Sig;
													Show2 = round(10**(-nsd)*_a3{i})*10**nsd;
												end;
										%end;
									%else
										%do;
											Show1 = put(_a2{i},10.&dec);
											Show2 = put(_a3{i},10.&dec);
										%end;
									if dv = 0 then _a7{i} = trim(left(Show1))|| ' ('||trim(left(Show2))||')';
									   else do; _a2{i} = int(_a2{i}); _a7{i} = trim(left(put(_a1{i},10.)))||', '||Trim(left(put(_a2{i},mmddyy8.)))||' '||trim(left(put(_a3{i},10.1))); end;
									 if _a1{i} in(.,0) then _a7{i} = ''; else _nvalid = _nvalid + 1;
								end;
								Label Label2 = 'Variable';
								Label2 = trim(left(Label2))||', mean (SD)';
								%if &Total = R or &Total = RC %then
									%do;
										%if &Dec = A %then
											%do;
												if TMean = 0 then Show1 = '0.0';
												else
													do;
														nmean = floor(log10(abs(TMean))) + 1 - &Sig;
														Show1 = round(10**(-nmean)*TMean)*10**nmean;
													end;
												if TSD = 0 then Show2 = '0.0';
												else
													do;
														nsd = floor(log10(TSD)) + 1 - &Sig;
														Show2 = round(10**(-nsd)*TSD)*10**nsd;
													end;
											%end;
										%else
											%do;
												Show1 = put(TMean,10.&dec);
												Show2 = put(TSD,10.&dec);
											%end;
										if dv = 0 then
											total = trim(left(Show1))||' '||Byte(177)||' '||trim(left(Show2))||' (N='||trim(left(put(TN,10.)))||')';
										else
								...
								Keep Variable Label2 GroupVal1-GroupVal&NGroups PValue1 PValue2 Total n1-n&NGroups Show1-Show3;
								output;
							%end;
						%if &median = Y or &median = MM %then
							%do; 
								_nvalid = 0;
								do i = 1 to &NGroups;
									%if &Dec = A %then
										%do;
											if _a6{i} = 0 then Show1 = '0.0';
											else
												do;
													nmed = floor(log10(abs(_a6{i}))) + 1 - &Sig;
													Show1 = round(10**(-nmed)*_a6{i})*10**nmed;
												end;
											if _a4{i} = 0 then Show2 = '0.0';
											else
												do;
													nmin = floor(log10(abs(_a4{i}))) + 1 - &Sig;
													Show2 = round(10**(-nmin)*_a4{i})*10**nmin;
												end;
											if _a5{i} = 0 then Show3 = '0.0';
											else
												do;
													nmax = floor(log10(abs(_a5{i}))) + 1 - &Sig;
													Show3 = round(10**(-nmax)*_a5{i})*10**nmax;
												end;
										%end;
									%else
										%do;
											Show1 = put(_a6{i},10.&dec);
											Show2 = put(_a4{i},10.&dec);
											Show3 = put(_a5{i},10.&dec);
										%end;
								   if dv = 0 then
										do;
											if _a4{i} < 0 or _a5{i} < 0 then	_a7{i} = trim(left(Show1))||' ('||trim(left(Show2))||' to '||trim(left(Show3))||')';
											else _a7{i} = trim(left(Show1))||' ('||trim(left(Show2))||'-'||trim(left(Show3))||')';
										end;
								...
								%if &Total = R or &Total = RC %then
									%do;
										%if &Dec = A %then
											%do;
												if TMedian = 0 then Show1 = '0.0';
												else
													do;
														nmed = floor(log10(abs(TMedian))) + 1 - &Sig;
														Show1 = round(10**(-nmed)*TMedian)*10**nmed;
													end;
												if TMin = 0 then Show2 = '0.0';
												else
													do;
														nmin = floor(log10(abs(TMin))) + 1 - &Sig;
														Show2 = round(10**(-nmin)*TMin)*10**nmin;
													end;
												if TMax = 0 then Show3 = '0.0';
												else
													do;
														nmax = floor(log10(abs(TMax))) + 1 - &Sig;
														Show3 = round(10**(-nmax)*TMax)*10**nmax;
													end;
											%end;
										%else
											%do;
												Show1 = put(TMedian,10.&dec);
												Show2 = put(TMin,10.&dec);
												Show3 = put(TMax,10.&dec);
											%end;
										if dv = 0 then
											do;
												if TMin < 0 or TMax < 0 then Total = trim(left(Show1))||' ('||trim(left(Show2))||' to '||trim(left(Show3))||')';
												else Total = trim(left(Show1))||' ('||trim(left(Show2))||'-'||trim(left(Show3))||')';
											end;
								...
								do i = 1 to &NGroups;
									%if &Dec = A %then
										%do;
											...
											if _a8{i} = 0 then Show2 = '0.0';
											else
												do;
													nq1 = floor(log10(abs(_a8{i}))) + 1 - &Sig;
													Show2 = round(10**(-nq1)*_a8{i})*10**nq1;
												end;
											if _a9{i} = 0 then Show3 = '0.0';
											else
												do;
													nq2 = floor(log10(abs(_a9{i}))) + 1 - &Sig;
													Show3 = round(10**(-nq2)*_a9{i})*10**nq2;
												end;
										%end;
									%else
										%do;
											Show1 = put(_a6{i},10.&dec);
											Show2 = put(_a8{i},10.&dec);
											Show3 = put(_a9{i},10.&dec);
										%end;
								   if dv = 0 then
										do;
											if _a8{i} < 0 or _a9{i} < 0 then	_a7{i} = trim(left(Show1))||' ('||trim(left(Show2))||' to '||trim(left(Show3))||')';
											else _a7{i} = trim(left(Show1))||' ('||trim(left(Show2))||'-'||trim(left(Show3))||')';
										end;
									   else do; _a6{i} = int(_a6{i}); _a7{i} = trim(left(put(_a6{i},mmddyy8.)))||' ('||trim(left(put(_a8{i},mmddyy8.)))||'-'||trim(left(put(_a9{i},mmddyy8.)))||')'; end;
									if _a1{i} in(.,0) then _a7{i} = ''; 
									%if &Mean = N %then %do; else _nvalid = _nvalid + 1; %end;
								end;
								%if &Mean = N %then %do; if _nvalid <= 1 then do; PValue1 = ''; PValue2 = ''; end; %end;
									...
									%do;
										%if &Dec = A %then
											%do;
												...
												if TQ1 = 0 then Show2 = '0.0';
												else
													do;
														nq1 = floor(log10(abs(TQ1))) + 1 - &Sig;
														Show2 = round(10**(-nq1)*TQ1)*10**nq1;
													end;
												if TQ3 = 0 then Show3 = '0.0';
												else
													do;
														nq2 = floor(log10(abs(TQ3))) + 1 - &Sig;
														Show3 = round(10**(-nq2)*TQ3)*10**nq2;
													end;
											%end;
										%else
											%do;
												Show1 = put(TMedian,10.&dec);
												Show2 = put(TQ1,10.&dec);
												Show3 = put(TQ3,10.&dec);
											%end;
										if dv = 0 then
											do;
												if TMin < 0 or TMax < 0 then Total = trim(left(Show1))||' ('||trim(left(Show2))||' to '||trim(left(Show3))||')';
										...
										else do; TMedian = int(TMedian); Total = trim(left(put(TMedian,mmddyy8.)))||' ('||trim(left(put(TQ1,mmddyy8.)))||'-'||trim(left(put(TQ3,mmddyy8.)))||')'; end;
									%end;
								%if &Mean = Y %then %do; PValue1 = ''; PValue2 = ''; %end;
								output;
							%end;
		...
		%do m = 1 %to &NFreqVars;
         ...
         run;
			...
			* For some strange reason the %sysfunc(exist(_stats)) does not return 1 for the first categorical variable
			  even though _stats normally exists.  We will check that both the grouping variable and the freq variable
			  have at least two different values.;
			proc sort data = _counts; by &GroupVar;
			data _null_;
			   set _counts end = done; by &GroupVar;
				if _n_ = 1 then _NVals = 0;
				retain _NVals;
				if first.&GroupVar then _NVals = _NVals + 1;
				if done then call symput('NGps',_NVals);
			run;
			proc sort data = _counts; by &ThisFreqVar;
			data _null_;
			   set _counts end = done; by &ThisFreqVar;
				if _n_ = 1 then _NVals = 0;
				retain _NVals;
				if first.&ThisFreqVar and not(missing(&ThisFreqVar)) then _NVals = _NVals + 1;
				if done then call symput('NRows',_NVals);
			run;
						
			%Let PChi =;
			%if &NGps > 1 and &NRows > 1 %then
				%do;
			...
			* The array a1 in the next data step will cover the &NCat rows by &NGroups cols for this categorical variable;
			...
			data _freqs;
			   set _counts end = done; 
				Length Test $ 60 PValue1 PValue2 $ 7 Total $ 60  Label $ 80;
				...
				if done then
					do;
						...
						* Write an observation with the variable name and/or label;
						%if &Label = N %then 
							%do;
								Label = vname(&ThisFreqVar)||', n(%)';
							%end;
						%if &Label = L %then
							%do;
								Label = vlabel(&ThisFreqVar)||', n(%)';
							%end;
						%if &Label = NL %then
							%do;
								if vlabel(&ThisFreqVar) = vname(&ThisFreqVar) then Label = vname(&ThisFreqVar)||', n(%)'; else
									Label = vname(&ThisFreqVar)||', n(%)'' ||vlabel(&ThisFreqVar))';
							%end;
						do j = 1 to &NGroups;
						   _a4{j} = '';
						end;
	...
	* Get group Ns and put into macro variables;
	...
	%if &Out ne %then
		%do;
			* Get data set label if any;
			proc contents noprint data = &dsname out = _check;
			data _null_;
			   set _check (obs = 1);
				call Symput('_Label',MemLabel);
			run;
			%put Data Set Label is &_Label;
	...
	%ExitTable1:
   proc datasets nolist; delete _all _all2 _all3 _allnpar _check _counts _counts2 _freqs _gpvarcounts 
